...
@app.route('/verify', methods=['POST']) # FIX 1: Changed from /api
def verify():
    # security check
    ...
    if action == "check": # FIX 2: ADDED THIS
    ...
    if action == "activate":
        ...
        conn.commit()
        return jsonify({"status": "OK", "plan": lic['plan'], "credits_left": lic['credits'], "expires_at": lic['expires']}) # added expires_at
    ...
    if action == "use_credit":
        ...
        if lic['credits'] != -1: # -1 = 30 day unlimited
            ...
            conn.commit()
            return jsonify({"status": "OK", "credits_left": new_credits, "expires_at": lic['expires']}) # return new credits
