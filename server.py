...
def init_db():
    ...
    c = conn.cursor()
    c.execute("DROP TABLE IF EXISTS licenses") # WIPES OLD BROKEN TABLE
...
def verify():
    try:
        ...
        data = request.json
        key = data['key']; machine_id = data['machine_id']; action = data['action']
        conn = get_db(); c = conn.cursor()
        c.execute("SELECT * FROM licenses WHERE key=?", (key,))
        lic = c.fetchone()
        if not lic: return jsonify({"status": "INVALID"})
        if lic['expires'] and lic['expires'] != "Never":
            if datetime.datetime.now() > datetime.datetime.fromisoformat(lic['expires']):
                return jsonify({"status": "EXPIRED"})
        if action == "check": return jsonify({"status": "OK", "plan": lic['plan'], "credits_left": lic['credits'], "expires_at": lic['expires']})
        if action == "activate":
            if lic['machine_id'] and lic['machine_id'] != "ANY" and lic['machine_id'] != machine_id:
                return jsonify({"status": "ALREADY_USED"})
            c.execute("UPDATE licenses SET machine_id=? WHERE key=?", (machine_id, key)); conn.commit()
            return jsonify({"status": "OK", "plan": lic['plan'], "credits_left": lic['credits'], "expires_at": lic['expires']})
        if action == "use_credit":
            if lic['credits'] == 0: return jsonify({"status": "NO_CREDITS"})
            if lic['credits'] != -1:
                new_credits = lic['credits'] - 1; new_used = lic['total_used'] + 1
                c.execute("UPDATE licenses SET credits=?, total_used=? WHERE key=?", (new_credits, new_used, key)); conn.commit()
                return jsonify({"status": "OK", "credits_left": new_credits, "expires_at": lic['expires']})
            return jsonify({"status": "OK", "credits_left": -1, "expires_at": lic['expires']})
...
{% for lic in licenses %}
<tr>
  <td>{{lic.key}}</td><td>{{lic.plan}}</td>
  <td>{{'Unlimited' if lic.credits == -1 else lic.credits}}</td>
  <td>{{'Never' if not lic.expires or lic.expires == 'Never' else lic.expires[:10]}}</td>
  <td>{{lic.machine_id or 'None'}}</td><td>{{lic.total_used}}</td>
</tr>
{% endfor %}</table>
"""
...
def admin():
    ...
    if request.method == 'POST':
        ...
        if plan == "5 Merges": credits, prefix, expires = 5, "EMP-5", "Never"
        elif plan == "10 Merges": credits, prefix, expires = 10, "EMP-10", "Never"
